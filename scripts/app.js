...
/*
 * Tạm thời tắt chức năng hai ngôn ngữ.
 * Đổi thành true khi muốn bật lại EN/VI.
 */
...
function translate(key) {
  return (
    window.PORTFOLIO_TRANSLATIONS?.[currentLanguage]?.[key] ||
    key
  );
}
...
/*
 * Khi tắt ngôn ngữ, nội dung trong các file HTML
 * sẽ được giữ nguyên và không bị i18n.js ghi đè.
 */
function applyTranslations(root = document) {
  if (!ENABLE_LANGUAGE_SWITCH) return;
...
async function showRoute(route, pushState = true) {
  ...
  await new Promise(resolve => {
    setTimeout(resolve, activeRoute ? 180 : 0);
  });
...
function projectCard(project, index) {
  return `
    <button
      class="project-card card-enter game-press"
    ...
    >
      ...
      <span class="project-rarity">
        ${localized(project.categoryLabel) || project.category}
      </span>

      <span class="project-play">
        <svg>
          <use href="#icon-play"/>
        </svg>
      </span>
      ...
      <span class="project-info">
        <span>
          ...
          <small>${localized(project.subtitle)}</small>
        </span>
  ...
  document
    ...
    .forEach(chip => {
      chip.classList.toggle(
        'is-active',
        chip.dataset.filter === activeProjectFilter
      );
...
function openProject(id) {
  const project = window.PORTFOLIO_PROJECTS.find(
    item => item.id === id
  );
  ...
  const action = project.videoUrl
    ? `
      <a
        class="game-button yellow game-press"
        href="${project.videoUrl}"
        target="_blank"
        rel="noopener"
      >
        ${translate('popup.watch')}
...
/*
 * Xoá nút EN/VI khỏi giao diện khi chức năng
 * chuyển ngôn ngữ đang được tắt.
 */
if (!ENABLE_LANGUAGE_SWITCH) {
